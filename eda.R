...
### Create plot of each period of time the train was on 
# to see trends of the data in between supposed CIPs
...
for (num in 1:nrow(Change_Data)){
  ...
  if (num == 1){
    if (Change_Data$Time_ON[num] > 100) {
      plot(data_indices, list, main = "Train 3 Specific Flux Over Time
           Each cycle represents time period between assumed CIP",type = "l",col = colors[num], ylim = c(.075,.09), xlab = "Time (Hours)", ylab = "Specific Flux")
