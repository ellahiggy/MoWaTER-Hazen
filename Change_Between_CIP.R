...
plot(Train_3$Sp_Fl ~ Train_3$Date_Time)

for (num in 2:nrow(Train3_ON_Change)){
    time_frame <- c(Train_3$Date_Time[Train3_ON_Change$End_Index[num -1]],Train_3$Date_Time[Train3_ON_Change$Start_Index[num]])
    if (difftime(time_frame[2],time_frame[1],hours) > 2){
     abline(v = time_frame, col = "magenta") 
    }
